...
``.ana`` files of the 100 000 runs by ``scripts/narval/extract_catpa_part.py`` +
``scripts/rorqual/concat_catpa.py``, which skip the 24 header lines, i.e. also the
``2015.000`` initial-state row: day 0 is 2015-01-01, day 3286 is 2023-12-31. Mapping
(:data:`LHS_ANA_COLUMNS`, 1-based ``.ana`` columns):
...
def load_catpa_params(path: str | Path | None = None, *, canonical: bool = False) -> pd.DataFrame:
    ...
    :func:`agri_jax.io.rzwqm.params.canonical_name` (``"Pore Size (c2)"`` -> ``lam_1``).
    """

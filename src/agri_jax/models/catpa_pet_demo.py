"""CA-TPA PET demo: the first real :class:`~agri_jax.core.Model` assembled from existing processes.
...
1. :func:`sw_pet_from_forcing` -- thin wrapper of
   :func:`agri_jax.processes.pet.shuttleworth_wallace` that reads the canopy from the forcing;
...
from agri_jax.core.state import Forcing, State, field
...
def sw_pet_from_forcing(state: DemoState, params: PETSiteParams, forcing_t: DemoForcing) -> DemoState:
    ...
    Thin wrapper of :func:`agri_jax.processes.pet.shuttleworth_wallace`: green = total LAI,
...
def build_forcing(met: pd.DataFrame, ana: Any, start: str, end: str) -> DemoForcing:
    ...
    ``met`` is :func:`agri_jax.io.rzwqm.read_met` with ``prepare=True``. ``ana`` is
    :func:`agri_jax.io.rzwqm.read_ana` of a one-year run whose row 0 is the ``YYYY.000``

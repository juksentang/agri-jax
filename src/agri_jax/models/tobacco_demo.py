"""Tobacco-like organ-queue demo: leaves by rank on an :class:`~agri_jax.core.OrganQueue`, driven by an
:class:`~agri_jax.core.EventTable` (sow, topping, primings, final harvest).

A pipeline model, not a science model. It exercises the ``n_cohort`` axis (docs/en/02_architecture.md
section 3.7) end to end inside ``lax.scan``: appearance by thermal time, per-leaf growth, topping and
...
from agri_jax.core.organs import OrganQueue, age, aggregate, appear, grow, prime, top
from agri_jax.core.process import process
...
@process(
    ...
    source="bookkeeping; docs/en/02_architecture.md section 3.5 (rotation reuses the crop slot)",
)
...
@process(
    ...
    source="docs/en/02_architecture.md section 3.7 (topping caps n_active, priming harvests a rank range)",
)
def management(state: TobaccoState, params: TobaccoParams, forcing_t: TobaccoForcing) -> TobaccoState:
    ...
    Source: docs/en/02_architecture.md section 3.7; EventTable topping / priming / harvest fields.
    """
    ev = forcing_t.events
    c = state.crop
...
def build_forcing(met: pd.DataFrame, events: EventTable, start: str, end: str) -> TobaccoForcing:
    """Stack ``tmin`` / ``tmax`` of the prepared ``.MET`` (:func:`agri_jax.io.rzwqm.read_met`) over

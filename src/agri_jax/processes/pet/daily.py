...
signature so that they are registered (``agri_jax.core.registry``), linted with all five
rules, write-checked under ``AGRI_JAX_CHECK=1`` and can be composed in a
:class:`agri_jax.core.Model`.
...
reference model's forcing preparation (:func:`agri_jax.io.rzwqm.met.prepare_rzwqm_forcing`)
...
def pet_shuttleworth_wallace(state: PETState, params: PETSiteParams, forcing_t: DailyWeather) -> PETState:
    ...
    Thin wrapper of :func:`agri_jax.processes.pet.shuttleworth_wallace`; see that function for
...
def pet_asce_reference(state: PETState, params: PETSiteParams, forcing_t: DailyWeather) -> PETState:
    ...
    Thin wrapper of :func:`agri_jax.processes.pet.asce_reference_et` with the wind run converted
...
def pet_priestley_taylor(state: PETState, params: PETSiteParams, forcing_t: DailyWeather) -> PETState:
    ...
    Thin wrapper of :func:`agri_jax.processes.pet.priestley_taylor`.
